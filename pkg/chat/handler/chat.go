...
// HandleWebSocketConnection manages WebSocket communication
...
// 			if _, err := userClient.ViewProfile(ctx, &userpb.ID{ID: string(message.SenderID)}); err != nil {
// 				log.Println("Invalid UserID:", err)
// 				continue
// 			}
// 			if _, err := userClient.ViewProfile(ctx, &userpb.ID{ID: string(message.RecipientID)}); err != nil {
