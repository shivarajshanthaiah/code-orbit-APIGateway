...
func NewAdminRoute(c *gin.Engine, cfg config.Config) {
	...
	auth.Use(middleware.AdminAuthorization(cfg.SECRETKEY, "admin"))
	{
		...
		auth.POST("/add/subscription/plan", adminHandler.AddSubPlan)
		...
		auth.GET("/get/problems/stats", adminHandler.GetProblemStats)
		auth.GET("/get/leaderboard/stats", adminHandler.GetLeaderboardStats)
